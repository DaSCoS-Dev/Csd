try {
   if('undefined' == typeof xajax.debug)xajax.debug = {
      }
   }
catch(e) {
...
if('undefined' == typeof xajax.debug.windowStyle)xajax.debug.windowStyle = 'width=800,' + 'height=600,' + 'scrollbars=yes,' + 'resizable=yes,' + 'status=yes';
...
xajax.debug.writeMessage = function(text, prefix, cls) {
   try {
      ...
      if('undefined' == typeof xd.window || true == xd.window.closed) {
         ...
         if("about:blank" == xd.windowSource)xd.window.document.write(xd.windowTemplate);
         }
      var xdw = xd.window;
      var xdwd = xdw.document;
      if('undefined' == typeof prefix)prefix = '';
      if('undefined' == typeof cls)cls = 'debugText';
...
xajax.executeCommand = function(args) {
   try {
      if('undefined' == typeof args.cmd)throw {
         code : 10006};
      if('undefined' == typeof xajax.commands[args.cmd])throw {
         code : 10007, data : args.cmd};
...
xajax.tools.$ = function(sId) {
   try {
      ...
      if('object' != typeof returnValue)throw {
         code : 10008};
      }
   catch(e) {
      var msg = '$:';
...
xajax._internalSend = function(oRequest) {
   try {
      ...
      xajax.debug.writeMessage(xajax.debug.text[105] + oRequest.requestData.length + xajax.debug.text[106]);
...
if(xajax.tools) {
   if(xajax.tools.queue) {
      if(xajax.tools.queue.retry) {
         if('undefined' == typeof xajax.debug.tools)xajax.debug.tools = {
            };
         if('undefined' == typeof xajax.debug.tools.queue)xajax.debug.tools.queue = {
            };
         xajax.debug.tools.queue.retry = xajax.tools.queue.retry;
         xajax.tools.queue.retry = function(obj, count) {
            if(xajax.debug.tools.queue.retry(obj, count))return true;
