...
*/
xajax.config.setDefault('waitCursor', true);
...
*/
xajax.config.setDefault('statusMessages', true);
...
xajax.config.setDefault('siteTitle', document.title);
...
xajax.tools._getFormValue = function(aFormValues, child, submitDisabledElements, prefix)
{
	...
	if (child.name){
		...
		if (prefix != child.id.substring(0, prefix.length))
		return;
	} else {
		alert("Non posso identificare l'elemento " + child);
	}
	if (child.type)
...
*/
xajax.js.includeScriptOnce = function(fileName, divName) {
...
*/
xajax.js.includeScript = function(fileName, divName) {
	var oDoc = xajax.config.baseDocument;
...
xajax.commands['ino'] = function(args) {
	...
	return xajax.js.includeScriptOnce(args.data, args.dvn);
}
