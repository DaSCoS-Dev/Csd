...
function show_too_short(id, min_length){
	...
	setTimeout( function() { $('#' + id).val(string); $('#' + id).prop( "disabled", false ); $('#' + id).focus() }, 1500);
}
...
paypal.Buttons({
    ...
    onApprove: function(data, actions) {
      return actions.subscription.get().then(    		   
						function(orderData) { xajax_execute('Payments/Main_paypal',	'paypal_acquistato', orderData) }
				)
    },
...
function load_content(url){
	var iframe = document.getElementById("full_stat_content");iframe.src = url;
	}
...
function do_data_table(id_tabella, sorting, section, data_type, filter, total) {
	...
	if (total == undefined) {
	 ...
	 // Se non sto facendo la lista fatture, resetto la variabile
	 if (section != "fatture"){
		 fatture_selezionate = [];
	 } else {
		 check_fatture_selezionate(true);
	 }
	...
	data_table_rendered = $("#" + id_tabella).DataTable(
			{
				...
				sServerMethod : "POST",
				ajax : {
					url : "/ajax_requests/ajax_" + my_ajax_section + "/get_" + my_function
							+ "_tabella" + "/" + filter,
					dataType : 'json',
					type : "POST",
					data : function(d) {
