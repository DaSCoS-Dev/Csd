...
function remove_returns( $stringa ) {
...
function sanitize_for_js( $stringa, $complete = true ) {
	if ($complete) {
		$replacement = "<br />";
	} else {
		$replacement = "";
	...
	// Sanifico le virgolette singole ( ' )
	$sanitized = str_replace( array (
			"'" 
	), "&apos;", $sanitized );
	if ($complete) {
		$sanitized = htmlentities( iconv( "UTF-8", "ISO-8859-1", $sanitized ), ENT_QUOTES | ENT_IGNORE | ENT_HTML5 );
	}
	return $sanitized;
}
