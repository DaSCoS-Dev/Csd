...
class CI_Security {
	...
	public function xss_clean( $str, $is_image = FALSE ) {
		...
		do {
			...
			// data:image;base64, è un'immagine raw che prendo come buona
			$has_raw = false;
			preg_match_all( "/<img src=\"data:image;base64,(.*?)\"/i", $str, $matchraw );
			if ($matchraw [ 1 ] [ 0 ] !== "") {
				// Replace dell'identificativo con "trappola"
				$str = str_ireplace( "data:image;base64,", "##replacer_raw##", $str );
				$has_raw = true;
			}
			if (preg_match( "/<img/i", $str, $match1 )) {
				$str = preg_replace_callback( "#<img\s+([^>]*?)(\s?/?>|$)#si", array (
						$this,
						'_js_img_removal' 
				), $str );
			}
			// Rimetto a posto il raw
			if ($has_raw) {
				$str = str_ireplace( "##replacer_raw##", "data:image;base64,", $str );
			}
