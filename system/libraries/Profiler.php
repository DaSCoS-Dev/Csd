...
class CI_Profiler {
	...
	public function set_sections( $config ) {
		foreach ( $config as $method => $enable ) {
			if (in_array( $method, $this->_available_sections )) {
				$name = "_compile_{$method}";
				$this->$name = ( $enable !== FALSE ) ? TRUE : FALSE;
			}
		}
	}
	// --------------------------------------------------------------------
	...
	protected function _compile_benchmarks( ) {
		...
		foreach ( $this->CI->benchmark->marker as $key => $val ) {
			...
			if (preg_match( "/(.+?)_exit/i", $key, $match )) {
				if (isset( $this->CI->benchmark->marker [ $match [ 1 ] . '_exit' ] ) and isset( $this->CI->benchmark->marker [ $match [ 1 ] . '_enter' ] )) {
					$profile [ $match [ 1 ] ] = $this->CI->benchmark->elapsed_time( $match [ 1 ] . '_enter', $key );
				}
			}
		}
