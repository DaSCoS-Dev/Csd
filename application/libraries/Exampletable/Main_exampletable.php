 ...
 */
class Main_exampletable extends Super_lib {
	...
	public function __construct( ) {
		...
		// Variables
		$this->model = $this->model_exampletable;
		$this->view = $this->main_exampletable_views;
	...
	private function listObjects( $input = nulll ) {
		...
		$this->response->assign( "sectionTitle", "innerHTML", "Exampletable List" );
		$this->show_records_table();
		// Trasformo in datatable....
		$this->response->script( "do_data_table('record_table', [[1, 'asc']], 'exampletable', [{ orderable: false, targets: [0] } {$visibility_options}], null )" );
	}
	...
	private function editObject( $uniqueId = 0 ) {
		if (intval( $uniqueId ) > 0) {
			$key_array = array (
					"PRIMARY" => array (
							"{$this->model_exampletable->getPrimaryIndex()->Column_name}" => intval($uniqueId)
					) 
			);
			$object = $this->model->get_record( $key_array, "record_by_primary" )[0];
		} else {
			$object = new Exampletable();
		}
		...
		if (intval( $uniqueId ) > 0) {
			$this->response->assign( "sectionTitle", "innerHTML", "Edit Exampletable" );
		} else {
			$this->response->assign( "sectionTitle", "innerHTML", "New Exampletable" );
		}
	...
	private function saveObject( $object = null ) {
		...
		if ($idDb !== false) {
			...
			$this->error( "Got errors during saving of Exampletable...", "Ko" );
		}
	}
}
?>