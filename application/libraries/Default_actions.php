...
class Default_actions extends Super_lib {
	...
	public function menu_iniziale( $is_refresh = false ) {
		...
		if (is_a( $this->response, "xajaxResponse" )) {
			...
			if ($this->config->config["framework_configured"] === false and $_SERVER["REMOTE_ADDR"] === "172.16.0.10"){
			...
			if ($this->is_logged_in()){
				$this->response->includeScriptOnce("https://cdn.datatables.net/1.13.4/js/jquery.dataTables.min.js");
				$this->response->includeScriptOnce("https://cdn.datatables.net/1.13.4/js/dataTables.bootstrap5.min.js");
				$this->response->includeCSS("https://cdn.datatables.net/1.13.4/css/dataTables.bootstrap5.min.css");
			}
