...
class xajaxPluginManager {
	...
	function generateClientScript( $asHtml = false ) {
		...
		foreach ( $aKeys as $sKey ) {
			// Check
			if (! isset( $this->aClientScriptGenerators [ $sKey ]->ci )) {
				$this->aClientScriptGenerators [ $sKey ]->ci = $this->ci;
			}
			$html .= $this->aClientScriptGenerators [ $sKey ]->generateClientScript( $asHtml );
		}
		if ($asHtml) {
			return $html;
		}
