...
/*
 * @package xajax
 * @version $Id: xajaxFunctionPlugin.inc.php,v 1.1 2013-02-11 00:49:46 developer Exp $
...
class xajaxFunctionPlugin extends xajaxRequestPlugin {
	...
	function register( $aArgs ) {
		if (1 < count( $aArgs )) {
			$sType = $aArgs [ 0 ];
			if (XAJAX_FUNCTION == $sType or $sType == "XAJAX_FUNCTION") {
				$xuf = & $aArgs [ 1 ];
			...
			if (0 < count( $this->aFunctions )) {
				if ($asHtml) {
					...
					foreach ( array_keys( $this->aFunctions ) as $sKey )
						$html .= $this->aFunctions [ $sKey ]->generateClientScript( $this->sXajaxPrefix, $asHtml );
					$html .= "/* ]]> */\n";
					$html .= "</script>\n";
				} else {
					echo "\n<script  " . $this->sDefer . ">\n";
					echo "/* <![CDATA[ */\n";
					foreach ( array_keys( $this->aFunctions ) as $sKey )
						$this->aFunctions [ $sKey ]->generateClientScript( $this->sXajaxPrefix, $asHtml );
					echo "/* ]]> */\n";
					echo "</script>\n";
				}
			}
