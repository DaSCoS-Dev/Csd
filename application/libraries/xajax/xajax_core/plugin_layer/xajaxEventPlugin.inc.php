...
/*
@package xajax
@version $Id: xajaxEventPlugin.inc.php,v 1.1 2013-02-11 00:49:46 developer Exp $
		...
		if (false === $this->bDeferScriptGeneration || 'deferred' === $this->bDeferScriptGeneration)
		{
			if (0 < count($this->aEvents))
			{
				if ($asHtml) {
					...
					$html .= $this->aEvents[$sKey]->generateClientScript($this->sXajaxPrefix, $this->sEventPrefix, $asHtml);
					...
					foreach (array_keys($this->aEvents) as $sKey)
					$this->aEvents[$sKey]->generateClientScript($this->sXajaxPrefix, $this->sEventPrefix, $asHtml);
