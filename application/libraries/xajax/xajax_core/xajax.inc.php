...
/*
 * @package xajax
 * @version $Id: xajax.inc.php,v 1.1 2013-02-11 00:49:25 developer Exp $
...
class xajax {
	 ...
	 */
	function printJavascript( $sJsURI = "", $aJsFiles = array(), $asHtml = false ) {
		if (0 < strlen( $sJsURI ))
			$this->configure( "javascript URI", $sJsURI );
		if (0 < count( $aJsFiles ))
			$this->configure( "javascript files", $aJsFiles );
		return $this->objPluginManager->generateClientScript( $asHtml );
	}
	 ...
	 * _uncompressed file with a similar name. This strdascoss out the
	...
	function autoCompressJavascript( $sJsFullFilename = NULL, $bAlways = false ) {
		$sJsFile = 'xajax_js/xajax_core.min.js';
		if ($sJsFullFilename) {
							...
							if (file_exists( $sPath )) {
								...
								foreach ( array_keys( $aFile ) as $sKey )
									if ('//SkipDebug' == substr( $aFile [ $sKey ], 0, 11 ))
										...
										-- $nSkip;
									else if ('<' . '?php' == substr( $aFile [ $sKey ], 0, 5 )) {} else if ('?' . '>' == substr( $aFile [ $sKey ], 0, 2 )) {} else if (0 == $nSkip)
										$aParsed [ ] = $aFile [ $sKey ];
								unset( $aFile );
	...
	function _decodeUTF8Data( $sData ) {
		$sValue = $sData;
		if ($this->aSettings [ "decodeUTF8Input" ]) {
			$sFuncToUse = NULL;
