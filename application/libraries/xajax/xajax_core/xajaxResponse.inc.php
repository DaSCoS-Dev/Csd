...
class xajaxResponse {
	 ...
	 * object - The <xajaxResponse> object.
	 */
	function new_window( $sURL, $iDelay = 0 ) {
		// we need to parse the query part so that the values are rawurlencode()'ed
		// can't just use parse_url() cos we could be dealing with a relative URL which
		// parse_url() can't deal with.
		$queryStart = strpos( $sURL, '?', strrpos( $sURL, '/' ) );
	 ...
	 * object - The <xajaxResponse> object.
	 */
	function includeScript( $sFileName, $divName = "" ) {
		return $this->addCommand( array (
				'n' => 'in',
				'dvn' => $divName 
		), $sFileName );
	 ...
	 * object - The <xajaxResponse> object.
	 */
	function includeScriptOnce( $sFileName, $divName = "" ) {
		return $this->addCommand( array (
				'n' => 'ino',
				'dvn' => $divName 
		), $sFileName );
	...
	function _printEscapedString_XML( &$sData ) {
		...
		if (0 < $nCDATA) {
			...
			/**
			 * Trick...se ho files e sono in xajax, ho problemi perche'
			 * il metodo per uppare files (xajax.upload) crea casino su cio' che
			 * ritorna come funzioni JS o come innerHTML ecc ecc
			 */
			if (is_string( $sData ) and sizeof( $_FILES ) === 0) {
				print 'S';
			} else if (is_int( $sData ) || is_float( $sData )) {
