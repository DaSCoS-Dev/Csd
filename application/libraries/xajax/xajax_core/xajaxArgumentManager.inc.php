...
/*
 * @package xajax
 * @version $Id: xajaxArgumentManager.inc.php,v 1.1 2013-02-11 00:49:25 developer Exp $
...
class xajaxArgumentManager {
	...
	function argumentDecodeXML( &$sArg ) {
								...
								if ($nLast < $nCurrent) {
									...
									switch ( $cType ) {
										case 'S' :
											...
											break;
										case 'B' :
											$mValue = $this->check_boolean( $sArg );
											break;
										case 'N' :
											$mValue = floatval( substr( $mValue, 1 ) );
											break;
										case '*' :
											$mValue = null;
											break;
				...
				if (0 == $nCurrent) {
					...
					switch ( $cType ) {
						case 'S' :
							...
							break;
						case 'B' :
							$sArg = $this->check_boolean( $sArg );
							break;
						case 'N' :
							$sArg = floatval( substr( $sArg, 1 ) );
							break;
						case '*' :
							$sArg = null;
							break;
		...
		trigger_error( $objLanguageManager->getText( 'ARGMGR:ERR:01' ) . $sExpecting . $objLanguageManager->getText( 'ARGMGR:ERR:02' ) . $sChunk, E_USER_ERROR );
	}

	function check_boolean( $sArg ) {
		if (is_string( $sArg )) {
			$sArg = substr( $sArg, 1 );
		}
		if (is_bool( $sArg )) {
			...
			if (trim( strtolower( $sArg ) ) === "false" or trim( strtolower( $sArg ) ) === "falso" or trim( $sArg ) === "0") {
				return false;
			} else {
				return true;
			}
	...
	function __construct( ) {
		...
		// get_magic_quotes_gpc REMOVED IN PHP8, always FALSE from 5.5...
		if (1 == 0)
			array_walk( $this->aArgs, array (
					&$this,
					'argumentStripSlashes' 
			) );
		array_walk( $this->aArgs, array (
				&$this,
				'argumentDecodeXML' 
