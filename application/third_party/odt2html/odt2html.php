...
function odt2html( $odt_file, $xml_string = NULL ) {
	...
	if ($_ophir_odt_import_conf [ "features" ] [ "list" ] === 0)
	...
	if ($_ophir_odt_import_conf [ "features" ] [ "table" ] === 0)
	...
	if ($_ophir_odt_import_conf [ "features" ] [ "table of contents" ] === 0)
	...
	while ( $xml->read() ) {
		...
		if ($xml->nodeType === XMLReader::END_ELEMENT) { // Handle a closing tag
			...
			continue;
		} elseif (in_array( $xml->nodeType, array (
				XMLReader::ELEMENT,
				XMLReader::TEXT,
			...
			switch ( $xml->name ) {
				...
				case "text:h" : // Title
					if ($_ophir_odt_import_conf [ "features" ] [ "header" ] === 0) {
						$xml->next();
						break;
					} elseif ($_ophir_odt_import_conf [ "features" ] [ "header" ] === 1)
						break;
				...
				case "text:a" :
					if ($_ophir_odt_import_conf [ "features" ] [ "link" ] === 0) {
						$xml->next();
						break;
					} elseif ($_ophir_odt_import_conf [ "features" ] [ "link" ] === 1)
						break;
				...
				case "draw:image" :
					if ($_ophir_odt_import_conf [ "features" ] [ "image" ] === 0) {
						$xml->next();
						break;
					} elseif ($_ophir_odt_import_conf [ "features" ] [ "image" ] === 1)
						break;
					
					$image_file = 'zip://' . $odt_file . '#' . $xml->getAttribute( "xlink:href" );
					if (isset( $_ophir_odt_import_conf [ "images_folder" ] ) && is_dir( $_ophir_odt_import_conf [ "images_folder" ] )) {
...
( $xml->name != "text:note" || $xml->nodeType != XMLReader::END_ELEMENT ) ) // Stop on </style:style>
{
				...
				case "office:annotation" :
					...
					} while ( ! ( $xml->name === "office:annotation" && $xml->nodeType === XMLReader::END_ELEMENT ) ); // End of the note
